function dashboard(sim, valid, scen; inf_p_start)
    ...
    menu_explo = Menu(left_layout[2, 1:2],
        options = zip([
                "Schorfheide-Chorin",
                "Hainich",
                "Schwäbische Alb",
    ...
    toggle_water_red = Toggle(left_layout[5, 2],
        active = true)

    Label(left_layout[6, 1], "include nutrient reduction?";
        tellwidth = true, halign = :right)
    toggle_nutr_red = Toggle(left_layout[6, 2],
        active = true)
    ...
    toggles_mowing = [Toggle(mowing_layout[i + 1, 1],
    ...
    Label(center_layout[1, 1:4], "Grazing (start date, end date, LD)";
        halign = :left)
    toggles_grazing = [Toggle(center_layout[i + 1, 1], active = false) for i in 1:2]
    ...
    ############# Nutrients
    slidergrid_nut = SliderGrid(center_layout[4, 1:4],
        (label = "Nutrient\nindex",
            range = 0.0:0.01:1.0,
            format = "{:.2f}",
            startvalue = 0.8))
    ...
    Label(rightupper_layout[1, 1], "OR: Plot ID";
        tellwidth = true, halign = :left)
    toggle_plotID = Toggle(rightupper_layout[1, 2],
        active = false,
        tellwidth = false,
        halign = :left)
    ...
    Label(rightlower_layout[1, 1], "Abiotic variable (right lower plot)";
        tellwidth = false, halign = :left)
    menu_abiotic = Menu(rightlower_layout[2, 1],
        options = zip([
                "Precipitation [mm d⁻¹]",
                "Potential evapo-\ntranspiration [mm d⁻¹]",
                "Air temperature [°C]\n",
    ...
    Label(rightlower_layout[3, 1], "Color/trait (right upper plot)";
        tellwidth = false, halign = :left)
    menu_color = Menu(rightlower_layout[4, 1],
        options = zip([
    ...
    sliders_param = SliderGrid(param_layout[2, 1],
        # height=900,
        tellheight = true,
        param_slider_prep...;)
    ...
    on(run_button.clicks) do n
        if !still_running
            ...
            if use_simulated_data
                ...
                grazing_start = [tb.stored_string.val for tb in tb_grazing_start][grazing_selected]
                grazing_end = [tb.stored_string.val for tb in tb_grazing_end][grazing_selected]
                grazing_intensity = [parse(Float64, tb.stored_string.val)
                                     for tb in tb_grazing_intensity][grazing_selected]
